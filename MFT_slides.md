...
mermaid_code: |
  flowchart TD
    ...
    Test --> Check3{局放/耐压测试}
    Check3 -->|Pass| Release[放行]
    Check3 -->|Fail| Reject[拒绝]
    Rework1 --> Wind
    Rework2 --> Varnish
    Release --> End[入库]
    Reject --> End2[报废]
    ...
    style Release fill:#10B981
    style Reject fill:#DC2626
```
...
mermaid_code: |
  sequenceDiagram
    ...
    Design->>Proto: 输出：设计参数、仿真报告
    Proto->>Field: 输出：验证报告、样机设备
    Field->>Data: 实时上传：KPI、故障、环境数据 (1h 频率)
    Data->>Data: 数据清洗、统计分析
    Data->>Standard: 输出：证据包 (6个月数据)
    Standard->>Design: 反馈：标准要求与改进建议
    Note over Field,Data: 采集频率：1 小时<br/>数据质量：完整性 ≥ 95%
    Note over Data,Standard: 证据包：效率、温升、PD、MTBF<br/>样本量：≥2个示范点 × 6个月
```
...
content_requirements:
  ...
  - "标注客户付费触点与长期价值点"
notes: "帮助商务团队设计付费模型"
# PLACEHOLDER DATA: visual-designer should refine as business model canvas
chart_config:
...
content_requirements:
  ...
  - "标注触发条件与责任人"
notes: "支持风险定期审查与拨款决策"
# PLACEHOLDER DATA: visual-designer should refine as 2x2 risk matrix
mermaid_code: |
  graph TD
    ...
    subgraph "低概率 / 高影响 — P1 风险"
      C[标准延误<br/>缓解：提前参与+示范数据]
      D[市场需求下滑<br/>缓解：多场景验证]
    end
    subgraph "高概率 / 低影响 — P2 风险"
      E[制造延误<br/>缓解：Buffer+备选工厂]
    end
    subgraph "低概率 / 低影响 — P3 监控"
      F[认证持续性<br/>缓解：定期复审]
    end
    style A fill:#DC2626
    style B fill:#DC2626
...
content_requirements:
  ...
  - "显示责任人和验收门槛"
notes: "用于项目启动与月度跟踪"
# PLACEHOLDER DATA: visual-designer should refine as Gantt-style timeline
mermaid_code: |
  gantt
    title 0-12个月实施计划
    dateFormat YYYY-MM
    section Phase 1: 立项
    项目立项与决策批准           :milestone, m1, 2026-02, 0d
    示范场景选定与合同               :active, 2026-02, 1M
    材料候选选定                     :active, 2026-02, 1M
    阶段1完成                         :milestone, m2, 2026-03, 0d
    section Phase 2: 样机
    样机设计与仿真                   :2026-03, 2M
    样机制造与验证                   :2026-05, 2M
    样机验收 (效率≥98%)            :milestone, m3, 2026-06, 0d
    section Phase 3: 示范
    示范安装与调试                   :2026-07, 1M
    示范运行与数据采集               :2026-08, 5M
    数据分析与报告                   :2026-12, 1M
    示范阶段完成 (6个月数据)      :milestone, m4, 2027-01, 0d
```
...
mermaid_code: |
  gantt
    ...
    dateFormat YYYY-MM
    section 示范扩展
    第二批示范场景选型             :2027-01, 2M
    扩展示范运行 (3个场景)         :2027-03, 4M
    section 工艺优化
    制造工艺放大验证               :crit, 2027-02, 3M
    SPC 系统建立与校准             :2027-04, 2M
    section 供应链
    主要材料供应协议谈判           :2027-01, 3M
    备份供应商认证                   :2027-04, 2M
    section 产线准备
    产线设备采购与安装             :crit, 2027-03, 4M
    首批试产 (FPY≥90%)              :milestone, 2027-06, 0d
    量产准备就绪                     :milestone, 2027-07, 0d
```
