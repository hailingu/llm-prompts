...
## 使用工作流
1. **会话初始化**：读取 `MEMORY.md` 获取全局上下文和用户偏好。根据当前任务，通过 `list_dir` 扫描 `memory/` 目录识别相关主题，读取对应的 `memory/<theme>/*.md` 获取主题上下文。
2. **构建内容**：根据当前场景选择合适的**记忆模板**，填充内容。
3. **任务结束/复盘**：提取有全局长期价值的信息，追加或更新到 `MEMORY.md`，并清理或归档已完结的主题记忆。
