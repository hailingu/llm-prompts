...
- **Explicit Node Dimensions (Anti-Stretching Bug)**: **CRITICAL**: Always provide explicit `width` and `height` attributes when invoking `graph.addNode({ width: 140, height: 60... })`. Moreover, for `html` shape definitions, NEVER use `width: '100%', height: '100%'` on the injected DOM element if it is placed inside an X6 `foreignObject`. X6 rendering in Safari/macOS causes explosive vertical stretching. 
  - **MANDATORY MAC OS FIX**: In the `html: (cell) => { ... }` block, you MUST dynamically read the node's geometry via `const size = cell.getSize();` and forcefully assign the absolute pixel dimensions. E.g.: `wrap.style.width = size.width + 'px'; wrap.style.height = size.height + 'px'; wrap.style.overflow = 'hidden';` Also, apply inline hardcoded `width: 100%; height: 100%; box-sizing: border-box;` in the inner HTML string. If you miss this, Safari will render giant blue ovals traversing the entire screen.
